EESchema Schematic File Version 4
LIBS:V3S_ESP32_Feather-cache
EELAYER 29 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 4
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 1250 1300 1500 900 
U 5BC231CB
F0 "PMIC Power Battery" 50
F1 "power.sch" 50
F2 "AVDD" O R 2750 1550 50 
F3 "DOVDD" O R 2750 1650 50 
$EndSheet
$Comp
L Mechanical:MountingHole_Pad MH1
U 1 1 5BC1A07F
P 650 3000
F 0 "MH1" H 750 3051 50  0000 L CNN
F 1 "MountingHole_Pad" H 750 2960 50  0000 L CNN
F 2 "MountingHole:MountingHole_2.5mm_Pad_Via" H 650 3000 50  0001 C CNN
F 3 "~" H 650 3000 50  0001 C CNN
	1    650  3000
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole_Pad MH2
U 1 1 5BC1A0DB
P 650 3500
F 0 "MH2" H 750 3551 50  0000 L CNN
F 1 "MountingHole_Pad" H 750 3460 50  0000 L CNN
F 2 "MountingHole:MountingHole_2.5mm_Pad_Via" H 650 3500 50  0001 C CNN
F 3 "~" H 650 3500 50  0001 C CNN
	1    650  3500
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole_Pad MH3
U 1 1 5BC1A147
P 1800 3000
F 0 "MH3" H 1900 3051 50  0000 L CNN
F 1 "MountingHole_Pad" H 1900 2960 50  0000 L CNN
F 2 "MountingHole:MountingHole_2.5mm_Pad_Via" H 1800 3000 50  0001 C CNN
F 3 "~" H 1800 3000 50  0001 C CNN
	1    1800 3000
	1    0    0    -1  
$EndComp
$Comp
L Mechanical:MountingHole_Pad MH4
U 1 1 5BC1A198
P 1800 3450
F 0 "MH4" H 1900 3501 50  0000 L CNN
F 1 "MountingHole_Pad" H 1900 3410 50  0000 L CNN
F 2 "MountingHole:MountingHole_2.5mm_Pad_Via" H 1800 3450 50  0001 C CNN
F 3 "~" H 1800 3450 50  0001 C CNN
	1    1800 3450
	1    0    0    -1  
$EndComp
$Comp
L OB-rescue:GND-OLIMEX_Power #PWR01
U 1 1 5BC1A241
P 650 3100
F 0 "#PWR01" H 650 2850 50  0001 C CNN
F 1 "GND" H 655 2927 50  0000 C CNN
F 2 "" H 650 3100 60  0000 C CNN
F 3 "" H 650 3100 60  0000 C CNN
	1    650  3100
	1    0    0    -1  
$EndComp
$Comp
L OB-rescue:GND-OLIMEX_Power #PWR02
U 1 1 5BC1A26F
P 650 3600
F 0 "#PWR02" H 650 3350 50  0001 C CNN
F 1 "GND" H 655 3427 50  0000 C CNN
F 2 "" H 650 3600 60  0000 C CNN
F 3 "" H 650 3600 60  0000 C CNN
	1    650  3600
	1    0    0    -1  
$EndComp
$Comp
L OB-rescue:GND-OLIMEX_Power #PWR03
U 1 1 5BC1A29D
P 1800 3100
F 0 "#PWR03" H 1800 2850 50  0001 C CNN
F 1 "GND" H 1805 2927 50  0000 C CNN
F 2 "" H 1800 3100 60  0000 C CNN
F 3 "" H 1800 3100 60  0000 C CNN
	1    1800 3100
	1    0    0    -1  
$EndComp
$Comp
L OB-rescue:GND-OLIMEX_Power #PWR04
U 1 1 5BC1A2CB
P 1800 3550
F 0 "#PWR04" H 1800 3300 50  0001 C CNN
F 1 "GND" H 1805 3377 50  0000 C CNN
F 2 "" H 1800 3550 60  0000 C CNN
F 3 "" H 1800 3550 60  0000 C CNN
	1    1800 3550
	1    0    0    -1  
$EndComp
$Sheet
S 3650 1300 1850 2650
U 5BC231CE
F0 "All V3s input output" 50
F1 "io.sch" 50
F2 "SDIO1_CLK" B R 5500 2350 50 
F3 "SDIO1_CMD" B R 5500 2450 50 
F4 "SDIO1_DAT1" B R 5500 2650 50 
F5 "SDIO1_DAT2" B R 5500 2750 50 
F6 "SDIO1_DAT3" B R 5500 2850 50 
F7 "SDIO1_DAT0" B R 5500 2550 50 
F8 "UART2_TX" O R 5500 2000 50 
F9 "UART2_RX" I R 5500 2100 50 
F10 "PB2" B R 5500 1550 50 
F11 "PB3" B R 5500 1650 50 
F12 "PE16" B R 5500 1750 50 
F13 "CAM_AVDD" I L 3650 1550 50 
F14 "CAM_DOVDD" I L 3650 1650 50 
F15 "USB_D_P" B L 3650 3750 50 
F16 "USB_D_N" B L 3650 3650 50 
F17 "UART0_RX" I L 3650 3500 50 
F18 "UART0_TX" O L 3650 3400 50 
F19 "RST" B L 3650 3300 50 
F20 "CC1" B L 3650 3100 50 
F21 "CC2" B L 3650 3200 50 
F22 "VBUS" B L 3650 3000 50 
$EndSheet
Wire Wire Line
	5500 2350 6500 2350
Wire Wire Line
	6500 2450 5500 2450
Wire Wire Line
	5500 2550 6500 2550
Wire Wire Line
	6500 2650 5500 2650
Wire Wire Line
	5500 2750 6500 2750
Wire Wire Line
	6500 2850 5500 2850
Wire Wire Line
	5500 1550 6500 1550
Wire Wire Line
	5500 1650 6500 1650
Wire Wire Line
	5500 2000 6500 2000
Wire Wire Line
	6500 2100 5500 2100
Wire Wire Line
	6500 1750 5500 1750
Wire Wire Line
	2750 1550 3650 1550
Wire Wire Line
	3650 1650 2750 1650
$Comp
L Connector:Conn_01x16_Male J?
U 1 1 5D309607
P 3950 6150
AR Path="/5BC231CE/5D309607" Ref="J?"  Part="1" 
AR Path="/5D309607" Ref="J2"  Part="1" 
F 0 "J2" H 4058 7031 50  0000 C CNN
F 1 "Header16" H 4058 6940 50  0000 C CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x16_P2.54mm_Vertical" H 3950 6150 50  0001 C CNN
F 3 "~" H 3950 6150 50  0001 C CNN
	1    3950 6150
	-1   0    0    -1  
$EndComp
$Comp
L Connector:Conn_01x12_Male J?
U 1 1 5D30960D
P 4500 6350
AR Path="/5BC231CE/5D30960D" Ref="J?"  Part="1" 
AR Path="/5D30960D" Ref="J6"  Part="1" 
F 0 "J6" H 4608 7031 50  0000 C CNN
F 1 "Header12" H 4608 6940 50  0000 C CNN
F 2 "Connector_PinSocket_2.54mm:PinSocket_1x12_P2.54mm_Vertical" H 4500 6350 50  0001 C CNN
F 3 "~" H 4500 6350 50  0001 C CNN
	1    4500 6350
	1    0    0    -1  
$EndComp
$Comp
L Connector:Conn_01x02_Male J?
U 1 1 5D309613
P 4500 5450
AR Path="/5BC231CE/5D309613" Ref="J?"  Part="1" 
AR Path="/5D309613" Ref="J4"  Part="1" 
F 0 "J4" H 4608 5631 50  0000 C CNN
F 1 "BAT" H 4608 5540 50  0000 C CNN
F 2 "Connector_JST:JST_PH_S2B-PH-K_1x02_P2.00mm_Horizontal" H 4500 5450 50  0001 C CNN
F 3 "~" H 4500 5450 50  0001 C CNN
	1    4500 5450
	1    0    0    -1  
$EndComp
$Comp
L power:+BATT #PWR0124
U 1 1 5D30AC9F
P 4700 5850
F 0 "#PWR0124" H 4700 5700 50  0001 C CNN
F 1 "+BATT" V 4715 5978 50  0000 L CNN
F 2 "" H 4700 5850 50  0001 C CNN
F 3 "" H 4700 5850 50  0001 C CNN
	1    4700 5850
	0    1    1    0   
$EndComp
$Comp
L power:-BATT #PWR0125
U 1 1 5D30B670
P 4700 5450
F 0 "#PWR0125" H 4700 5300 50  0001 C CNN
F 1 "-BATT" V 4715 5578 50  0000 L CNN
F 2 "" H 4700 5450 50  0001 C CNN
F 3 "" H 4700 5450 50  0001 C CNN
	1    4700 5450
	0    1    1    0   
$EndComp
Text Label 4700 5950 0    50   ~ 0
EN
Text Label 4700 6050 0    50   ~ 0
VBUS
Text Label 4700 6150 0    50   ~ 0
GPIO6
Text Label 4700 6250 0    50   ~ 0
GPIO5
Text Label 4700 6350 0    50   ~ 0
GPIO4
Text Label 4700 6450 0    50   ~ 0
GPIO3
Text Label 4700 6550 0    50   ~ 0
GPIO2
Text Label 4700 6650 0    50   ~ 0
GPIO1
Text Label 4700 6750 0    50   ~ 0
GPIO0
Text Label 4700 6850 0    50   ~ 0
I2C_SCL
Text Label 4700 6950 0    50   ~ 0
I2C_SDA
Text Label 3750 6950 2    50   ~ 0
FreePin
Text Label 3750 6850 2    50   ~ 0
UART_TX_D0
Text Label 3750 6750 2    50   ~ 0
UART_RX_D0
Text Label 3750 6650 2    50   ~ 0
MISO
Text Label 3750 6550 2    50   ~ 0
MOSI
Text Label 3750 6450 2    50   ~ 0
SCK
Text Label 3750 6350 2    50   ~ 0
AIN5
Text Label 3750 6250 2    50   ~ 0
AIN4
Text Label 3750 6150 2    50   ~ 0
AIN3
Text Label 3750 6050 2    50   ~ 0
AIN2
Text Label 3750 5950 2    50   ~ 0
AIN1
Text Label 3750 5850 2    50   ~ 0
AIN0
$Comp
L power:GND #PWR0126
U 1 1 5D30FF16
P 3750 5750
F 0 "#PWR0126" H 3750 5500 50  0001 C CNN
F 1 "GND" V 3755 5622 50  0000 R CNN
F 2 "" H 3750 5750 50  0001 C CNN
F 3 "" H 3750 5750 50  0001 C CNN
	1    3750 5750
	0    1    1    0   
$EndComp
$Comp
L power:+3.3V #PWR0127
U 1 1 5D310B6B
P 3750 5550
F 0 "#PWR0127" H 3750 5400 50  0001 C CNN
F 1 "+3.3V" V 3765 5678 50  0000 L CNN
F 2 "" H 3750 5550 50  0001 C CNN
F 3 "" H 3750 5550 50  0001 C CNN
	1    3750 5550
	0    -1   -1   0   
$EndComp
Text Label 3750 5450 2    50   ~ 0
RST
$Comp
L Connector:USB_C_Receptacle_USB2.0 J1
U 1 1 5D3504F1
P 2100 6150
F 0 "J1" H 2207 7017 50  0000 C CNN
F 1 "USB_C_Receptacle_USB2.0" H 2207 6926 50  0000 C CNN
F 2 "Connector_USB:USB_C_Receptacle_GCT_USB4085" H 2250 6150 50  0001 C CNN
F 3 "https://www.usb.org/sites/default/files/documents/usb_type-c.zip" H 2250 6150 50  0001 C CNN
	1    2100 6150
	1    0    0    -1  
$EndComp
$Sheet
S 6500 1300 2100 2600
U 5C7A02C5
F0 "WIFI, BT, RT" 50
F1 "esp32.sch" 50
F2 "EN" I L 6500 1650 50 
F3 "UART0_RX" I L 6500 2000 50 
F4 "UART0_TX" O L 6500 2100 50 
F5 "SDIO1_CLK" B L 6500 2350 50 
F6 "SDIO1_CMD" B L 6500 2450 50 
F7 "SDIO1_DAT1" B L 6500 2650 50 
F8 "SDIO1_DAT2" B L 6500 2750 50 
F9 "SDIO1_DAT3" B L 6500 2850 50 
F10 "IO2_PULL" B L 6500 1750 50 
F11 "SDIO1_DAT0" B L 6500 2550 50 
F12 "IO0_PULL" B L 6500 1550 50 
F13 "SD3" B R 8600 1600 50 
F14 "SD2" B R 8600 1700 50 
F15 "IO5" B R 8600 1800 50 
F16 "IO35" B R 8600 1900 50 
F17 "IO34" B R 8600 2000 50 
F18 "IO33" B R 8600 2100 50 
F19 "IO32" B R 8600 2200 50 
F20 "IO27" B R 8600 2300 50 
F21 "IO26" B R 8600 2400 50 
F22 "IO25" B R 8600 2500 50 
F23 "IO23" B R 8600 2600 50 
F24 "IO22" B R 8600 2700 50 
F25 "IO21" B R 8600 2800 50 
F26 "IO19" B R 8600 2900 50 
F27 "IO18" B R 8600 3000 50 
F28 "SENSOR_VP" B R 8600 3200 50 
F29 "SENSOR_CAPP" B R 8600 3300 50 
F30 "SENSOR_CAPN" B R 8600 3400 50 
F31 "SENSOR_VN" B R 8600 3100 50 
$EndSheet
Text Label 8600 2600 0    50   ~ 0
MOSI
Text Label 8600 2900 0    50   ~ 0
MISO
Text Label 8600 3000 0    50   ~ 0
SCK
Text Label 8600 2700 0    50   ~ 0
I2C_SCL
Text Label 8600 2800 0    50   ~ 0
I2C_SDA
Text Label 8600 2500 0    50   ~ 0
AIN1
Text Label 8600 2400 0    50   ~ 0
AIN0
Text Label 8600 3400 0    50   ~ 0
AIN5
Text Label 8600 3300 0    50   ~ 0
AIN4
Text Label 8600 3200 0    50   ~ 0
AIN3
Text Label 8600 3100 0    50   ~ 0
AIN2
Text Label 8600 1800 0    50   ~ 0
FreePin
$Comp
L power:+BATT #PWR0130
U 1 1 5D37A56C
P 4700 5550
F 0 "#PWR0130" H 4700 5400 50  0001 C CNN
F 1 "+BATT" V 4715 5678 50  0000 L CNN
F 2 "" H 4700 5550 50  0001 C CNN
F 3 "" H 4700 5550 50  0001 C CNN
	1    4700 5550
	0    1    1    0   
$EndComp
Text Label 3650 3400 2    50   ~ 0
UART_TX_D0
Text Label 3650 3500 2    50   ~ 0
UART_RX_D0
Text Label 5850 6500 3    50   ~ 0
I2C_SCL
Text Label 6200 6500 3    50   ~ 0
I2C_SDA
$Comp
L Device:R R3
U 1 1 5D37C8B3
P 5850 6350
F 0 "R3" H 5920 6396 50  0000 L CNN
F 1 "10K" H 5920 6305 50  0000 L CNN
F 2 "Resistor_SMD:R_0201_0603Metric" V 5780 6350 50  0001 C CNN
F 3 "~" H 5850 6350 50  0001 C CNN
	1    5850 6350
	1    0    0    -1  
$EndComp
$Comp
L Device:R R5
U 1 1 5D37CFFA
P 6200 6350
F 0 "R5" H 6270 6396 50  0000 L CNN
F 1 "10K" H 6270 6305 50  0000 L CNN
F 2 "Resistor_SMD:R_0201_0603Metric" V 6130 6350 50  0001 C CNN
F 3 "~" H 6200 6350 50  0001 C CNN
	1    6200 6350
	1    0    0    -1  
$EndComp
$Comp
L power:+3.3V #PWR0132
U 1 1 5D37D598
P 6200 6200
F 0 "#PWR0132" H 6200 6050 50  0001 C CNN
F 1 "+3.3V" H 6215 6373 50  0000 C CNN
F 2 "" H 6200 6200 50  0001 C CNN
F 3 "" H 6200 6200 50  0001 C CNN
	1    6200 6200
	1    0    0    -1  
$EndComp
$Comp
L power:+3.3V #PWR0135
U 1 1 5D37E376
P 5850 6200
F 0 "#PWR0135" H 5850 6050 50  0001 C CNN
F 1 "+3.3V" H 5865 6373 50  0000 C CNN
F 2 "" H 5850 6200 50  0001 C CNN
F 3 "" H 5850 6200 50  0001 C CNN
	1    5850 6200
	1    0    0    -1  
$EndComp
Text Label 8600 1600 0    50   ~ 0
GPIO6
Text Label 8600 1700 0    50   ~ 0
GPIO5
Text Label 8600 1900 0    50   ~ 0
GPIO4
Text Label 8600 2000 0    50   ~ 0
GPIO3
Text Label 8600 2100 0    50   ~ 0
GPIO2
Text Label 8600 2200 0    50   ~ 0
GPIO1
Text Label 8600 2300 0    50   ~ 0
GPIO0
NoConn ~ 3750 5650
Text Label 3650 3100 2    50   ~ 0
CC1
Text Label 3650 3200 2    50   ~ 0
CC2
Text Label 2700 5750 0    50   ~ 0
CC1
Text Label 2700 5850 0    50   ~ 0
CC2
Text Label 2700 6100 0    50   ~ 0
USB_D_N
Text Label 2700 6300 0    50   ~ 0
USB_D_P
Wire Wire Line
	2700 6050 2700 6150
Wire Wire Line
	2700 6250 2700 6350
Text Label 3650 3650 2    50   ~ 0
USB_D_N
Text Label 3650 3750 2    50   ~ 0
USB_D_P
Text Label 3650 3000 2    50   ~ 0
VBUS
Text Label 2700 5550 0    50   ~ 0
VBUS
NoConn ~ 2700 6650
NoConn ~ 2700 6750
$Comp
L power:GND #PWR0141
U 1 1 5D397247
P 1950 7050
F 0 "#PWR0141" H 1950 6800 50  0001 C CNN
F 1 "GND" H 1955 6877 50  0000 C CNN
F 2 "" H 1950 7050 50  0001 C CNN
F 3 "" H 1950 7050 50  0001 C CNN
	1    1950 7050
	1    0    0    -1  
$EndComp
Wire Wire Line
	1800 7050 1950 7050
Connection ~ 1950 7050
Wire Wire Line
	1950 7050 2100 7050
Text Notes 1500 1800 0    50   ~ 0
TODO :\nCam power\nGeneral 3.3V and 5V\nDDR2 power
Text Notes 4000 2050 0    50   ~ 0
TODO :\nAnalog Audio
$EndSCHEMATC
